...
//, UISearchResultsUpdating, UITableViewDataSource, UITableViewDelegate
{
    ...
    @IBOutlet weak var tempNowLable: UILabel!
...
//    }
    ...
    func getRequestForSomyCity() {
        
        print(myCity.text!)
        if myCity.text! != "" {
            RequestSetting.shared.getWeather(city: "minsk", result: { (model) in
                if model != nil {
                        self.offerModel = model
                }
            })
    ...
    func putInfoInLabels() {
        tempNowLable.text = self.offerModel.city!.name
    }
    
    @IBAction func actionGetRequest(_ sender: Any) {
        getRequestForSomyCity()
        putInfoInLabels()
    }
}
